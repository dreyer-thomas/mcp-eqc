...
export function registerOpenIssuesTools(server: McpServer) {
  // Tool 1: Issues holen
  server.registerTool(
    "ec_open_issues",
    {
      ...
      description:
        "Listet offene Issues einer Hierarchie (things/types/hierarchies/type_hierarchies) mit optionalen Filtern (qp) & Pagination (step).",
      inputSchema: {
    ...
    async ({ hierarchy_type, hierarchy_id, qp, step }, _extra) => {
      ...
      const url = new URL(
        `${base}/openissues/v1/${hierarchy_type}/${encodeURIComponent(hierarchy_id)}/issues`
      );
      if (qp) url.searchParams.set("qp", JSON.stringify(qp));
  ...
  server.registerTool(
    "ec_issue_detail",
    {
      ...
      description:
        "Lädt die Details eines Issues über /openissues/v1/issues/:issue.",
      inputSchema: {
    ...
    server.registerTool(
      "ec_issue_attachments_list",
      {
        title: "EquipmentCloud: Issue-Attachments auflisten",
        description:
          "Liest die Liste der Attachments eines Issues über /openissues/v1/issues/:issue/attachments.",
        inputSchema: {
