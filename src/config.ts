...
export function ensureConfig(): string[] {
  ...
  if (!EC_CLOUD_BASE_URL) missing.push("EC_CLOUD_BASE_URL");
  if (!EC_USER) missing.push("EC_USER");
  if (!EC_PASS) missing.push("EC_PASS");
  return missing;
}

export function buildBasicAuth(): string {
  if (!EC_USER || !EC_PASS) throw new Error("EC_USER/EC_PASS nicht gesetzt");
