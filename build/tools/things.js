...
const normalize = (s) => s
    ...
    .trim()
    .replace(/\s+/g, " ");
const containsNorm = (hay, needle) => normalize(hay).includes(normalize(needle));
const startsWithNorm = (hay, needle) => normalize(hay).startsWith(normalize(needle));
export function registerThingsTools(server) {
    // A) Alle Things abrufen
    server.registerTool("ec_things_list", {
    ...
    server.registerTool("ec_thing_resolve", {
        title: "EquipmentCloud: Thing-Namen auflösen",
        description: "Sucht in /equipmenthub/v1/things nach Namen und gibt passende IDs zurück. Unterstützt exakte und partielle Matches.",
        inputSchema: {
        ...
        let results = items.filter((t) => {
            if (!t?.name)
                return false;
            // Name-Match
            const nameMatch = match_mode === "exact"
            ...
            if (hNeedle) {
                const hMatch = containsNorm(t.hierarchy ?? "", hNeedle) ||
    ...
    server.registerTool("ec_open_issues_by_thing_name", {
        title: "EquipmentCloud: Open Issues per Thing-Name",
        description: "Sucht Thing per Name (exact/icontains/startswith) und lädt anschließend dessen Open Issues (optional mit qp/step).",
        inputSchema: {
        ...
        const items = Array.isArray(tr.data?.items) ? tr.data.items : [];
        ...
        if (qp)
            issuesUrl.searchParams.set("qp", JSON.stringify(qp));
        if (step)
            issuesUrl.searchParams.set("step", String(step));
        // 3) Issues laden
